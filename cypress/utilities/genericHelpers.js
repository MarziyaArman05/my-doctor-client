class WebElement {
  ...
  elementIsPresent(element) {
    ...
    try {
      ...
      cy.log(
        "There was an Exception in checking whether the element is Present."
      );
    }
  }
  ...
  elementIsDisplayed(element) {
    try {
      ...
      cy.log(
        "There was an Exception in checking whether the element is Displayed."
      );
    }
  }
  ...
  shouldHaveCSS(element, cssname, value) {
    element.should("have.css", cssname, value).then(
      function (text) {
        cy.log("The element have css value: " + value);
      },
      function (err) {
        cy.log(
          "--->Error: The element dosn't have css expected css value due to: " +
            err
        );
      }
    );
  ...
  getAttribute(element, attribute) {
    element.invoke("attr", attribute).then(
      function (text) {
      ...
      function (err) {
        cy.log(
          "--->Error: The attribute of the element couldn't be captured due to: " +
            err
        );
      }
    );
  ...
  drag(sourceElement, targetElement) {
    cy.get(sourceElement)
      ...
      .then(
        function () {
          cy.log(
            "Element is dragged from " + sourceElement + " to " + targetElement
          );
        },
        function (err) {
